...
I1 (3): Konfiguriert über nftables mit \texttt{policy drop} auf Input und Forward (\path{modules/networking.nix}). \ac{SSH} ist rate-limitiert und existiert kein nach außen geöffneter Port durch \ac{DNS}-01-Zertifikaten.
...
\end{xltabular}
\caption{Indikatorbasierte Detailbewertung der Eigenentwicklung in den Basis-Kategorien}
...
Zu nennen ist, dass Szenario B in einem Aspekt formal sauberer als die Eigententwicklung arbeitet. B's Deny-by-default-Kommunikationsmatrix dokumentiert jeden erlaubten Pfad mit Quelle, Ziel und Port, während die Eigenentwicklung ihre real erzwungenen Pfade nur in einer vereinfachten Netzübersicht ausweist und dementsprechend eine stärkere Dokumentation liefert.
