...
Bei der Installation von NixOS werden die folgenden Warnmeldung ausgegeben:
\begin{itemize}
    \item "Mount point '/boot' which backs the random seed file is world accessible, which is a security hole! ..."
    \item "Random seed file '/boot/loader/.#bootctlrandom-seedc5706ea22f8895b0' is world accessible, which is a security hole! ..."
\end{itemize}
...
Die Sparrow Konfiguration wird in Abbildung \ref{fig6:sparrow_multiSetup} dargestellt.
...
Um den in Abbildung \ref{sec:cold_setup} dargestellten Prozess zu vereinfachen, stehen auf den Cold- und Hot-Wallet-\acp{VM} folgende Skripte zur Verfügung:
\begin{itemize}
...
Folgende Skripte, einsahbar in Tabelle \ref{tab:wireguard_scripts}, zum automatisierten Installieren von WireGuard auf dem Basissystem und zur Peer-Verbindung auf beiden Systemen wurden aufgesetzt:
...
Verschiedene Parameter wurden für die Standard-Hot-Wallet-\ac{PSBT}-Erstellung festgelegt und können vom Mandaten in der Applikationslogik angepasst werden.  Diese können in Tabelle \ref{tab:psbt_params} nachvollzogen werden.
\begin{table}[h]
\centering
...
Für Nachbefüllungs- und Sicherungs-Intents bestimmt der \ac{OPA} diese Parameter-Werte dynamisch auf Basis eines Risk Scores. Wie dieser Score aus dem Wallet-Saldo bestimmt wird und welche konkreten \ac{PSBT}-Parameter dieser festlegt, zeigt Tabelle~\ref{tab:OPA_fundSwitch}.
