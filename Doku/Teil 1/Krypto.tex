...
den vollständigen \ac{PSBT}-Workflow nach \cite{bip174}. 
Außerdem wird eine Anbindung an einen eigene Bitcoin-Core- oder Electrum-Node ermöglicht, welche wir nicht nutzen, jedoch als Alternative anerkennen (siehe \ref{XXX}). 
Es unterstützt zudem die Besonderheit der Cold-Wallet \acp{VM}, sodass es offline Signierung sowohl über Dateien, \ac{USB} und animierte \ac{QR}-Codes ermöglicht. Daneben zeichnet es sich ebenfalls durch eine breite Hardware-Wallet-Integration und eine Vielzahl an Konnektoren und Wallet Formaten aus, welche als weitere Alternativen beschrieben wurden und dem Mandaten offenstehen (Siehe \ref{XXX}).
Ein besonderes Merkmal ist die transparente Darstellung aller Transaktions- und \ac{UTXO}-Details, die die manuelle Verifikation im Cold-Prozess erst sicher und praktikabel macht und dementsprechend fest eingeplant ist \ref{XXX}\cite{sparrow_docs,Sparrow_Wallets_Features}.
...
\textbf{Alternativen zu Sparrow Wallet}
...
\begin{itemize}
  \item \textbf{Electrum:} Benötigt in der Online-Variante standardmäßig einen öffentlichen Electrum-Server und legt dadurch Wallet-Metadaten gegenüber Dritten offen. Dementsprechend bietet es eine geringere Privatsphäre als Sparrow Wallet für den beschriebenen alternativen Broadcast-Ansatz (siehe \ref{XXX}). Zudem verfügt es im Offline-Modus über eingeschränkte Funktionen und besitzt beispielsweise keine Wallet-übergreifende Auflösung von Zieladressen zu bekannten Wallets und eignet sich daher nur bedingt für eine sicherheitskritische, manuelle Offline-Verifikation (siehe \ref{XXX}). Daneben verwendet es historisch eigene, nicht Deskriptor-native Schlüssel- und Ableitungskonventionen, die die Interoperabilität mit dem in  Bitcoin Core importierten \path{wsh}-Deskriptor erschweren \cite{electrum_docs}.  
  ...
  Ein möglicher gekoppelter Betrieb mit Bitcoin Core ermöglicht offline das Registrieren des Schlüsselmaterials in Bitcoin Core und eine GUI Analyse mit Specter. Diese Kombination zeichnet sich jedoch durch eine signifikant größere Codebasis und damit einer erweiterten Angriffsfläche im Vergleich zu Sparrow aus. Zudem bleibt die Software in der \ac{PSBT}-Detail Tiefe hinter Sparrow zurück und eignet sich ebenfalls als Teilservice nicht \cite{specter_devices, specter_faq_hotwallet, knowingbitcoin_coordinators}.
...
Die air-gapped Cold-\acp{VM} benötigen ohnehin keinen Server, da sie die Transaktionsdetails ausschließlich aus der \ac{PSBT} selbst verifizieren. Die Möglichkeit des Anschluss von Sparrow zu Bitcoin Core als alternativer Broadcast Schritt bleibt dem Mandanten offen. \ref{XXX}.
...
Das Skript \path{wgHMAC_export.sh} kann auf der \ac{VM} von Key~A ausgeführt werden, um eine automatische Ordnerstruktur anzulegen und eine \path{meta.json} mit allen benötigten Informationen zu extrahieren. Für Key~C muss dies manuell vorgenommen werden, da die Schlüssel von Sparrow direkt verwaltet werden. Wir empfehlen das Anlegen eines Ordners \path{/wallets/cold} zur Ablage des Key~C-Deskriptors. Diese müssen zuletzt auf der Key-B-Maschine extrahiert und als Bestandteile für ein neues 2-aus-3-Multi-Signatur-Wallet importiert werden. Der zweite Schlüssel von Key~B kann direkt in derselben \ac{VM} kopiert werden.

Der daraus generierte wsh-Deskriptor soll anschließend als separate Datei auf dem Wechselmedium unter \path{/Wallets/Cold} als \path{cold-signer.descriptor} abgelegt werden, um später automatisch auf dem Basis-System importiert werden zu können.

Es empfiehlt sich ebenfalls, Key~A als Watch-Only-Wallet direkt in Sparrow als separate Wallet zu registrieren. Dadurch werden dynamische Adressen, die zu diesem Wallet gehören, automatisch aufgelöst und als Zieladresse der Transaktion angezeigt. Transaktionen an andere Wallets dürfen nicht signiert werden.
...
Nach dem Umstecken auf die Key-B-\ac{VM} muss der Operator die \ac{PSBT} in das Key-B-Wallet importieren, die Details verifizieren und die \ac{PSBT} signieren. Auf derselben \ac{VM} ist daraufhin eine Finalisierung im Cold-Wallet möglich.
Danach kann das \ac{USB}-Medium wieder in das Hot-Wallet eingesteckt und über das Skript \path{psbt_broadcast.sh} importiert sowie automatisch über Bitcoin Core finalisiert und gebroadcastet werden.
...
\hline
COLD\_STARTED & Der Operator hat die für den manuellen Cold-Workflow generierte \ac{PSBT} auf ein Wechselmedium extrahiert. \\
\hline
...
\hline
\end{tabular}
\caption{Geplante Subject-Berechtigungen je Microservice}
...
\begin{itemize}
    \item \path{/ops/refill/psbt_export.sh}: exportiert die letzte Cold-\ac{PSBT} auf ein Wechselmedium unter \path{/psbt/unappr.<id>.psbt} und erfüllt die Voraussetzung für den Cold-Signierungsprozess.
    \item \path{/ops/refill/psbt_broadcast.sh}: importiert eine Cold-\ac{PSBT} von einem Wechselmedium aus \path{/psbt/appr.<id>.psbt}.
\end{itemize}
