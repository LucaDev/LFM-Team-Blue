...
\paragraph{Alternativen} \label{sec:alt_alg} \mbox{}\\
...
In Kombination mit \ac{NATS} wird \ac{HTTP} als \ac{API} für GET-Calls verwendet, um Informationen direkt aus einem anderen Container zu extrahieren. Die Transportwege sind dabei klar aufgeteilt. 
Über \ac{HTTP} laufen ausschließlich die Entgegennahme externer Anfragen (\texttt{\seqsplit{/api/v1/request/bip21}}, \texttt{\seqsplit{/api/v1/request/psbt}}), der Signier-Aufruf an die Key-A-\ac{VM} (\texttt{POST /sign}) sowie die Health- und Metrics-Endpunkte. Alle betreiberausgelösten internen Abläufe wie die manuelle \ac{PSBT}-Einreichung, der Abschluss des Wechselmedium-Exports und der Broadcast der signierten Cold-\ac{PSBT} werden hingegen über \ac{NATS} unter der Operator-Identität ausgelöst (\texttt{\seqsplit{psbt.submit.requested}}, \texttt{\seqsplit{refill.export.done}}, \texttt{\seqsplit{refill.broadcast.requested}}) und besitzen bewusst keinen \ac{HTTP}-Endpunkt. Eine frühere \ac{HTTP}-basierte Variante dieser Operator-Aufrufe wurde zugunsten einer einheitlichen, über die \ac{NATS}-Berechtigungen abgesicherten nicht-Internet exponierten Ereignissteuerung entfernt.
...
Neben den beiden externen Schnittstellen (\texttt{bip21}, \texttt{psbt}) und den intern durch \ac{OPA} ausgelösten Austausch-\ac{TX} (\texttt{OPA\_hot}, \texttt{OPA\_cold}) existiert ein manueller Operator-Pfad (\texttt{rail == "manual"}), über den der Betreiber eine eigene, bereits fertig erstellte \ac{PSBT} einreichen kann. Es wird empfohlen diese in einer weiteren Sparrow Instanz mit dem Hot-Schlüssel als Watch-Only-Wallet zu bauen und soll über das Skript \path{scripts/ops/psbt_submit.sh} eingereicht.

Ausgelöst wird der Pfad nicht über \ac{HTTP}, sondern über das \ac{NATS}-Subject \texttt{\seqsplit{psbt.submit.requested}} unter der dedizierten Operator-Identität. Die eingereichte \ac{PSBT} wird unmittelbar als \texttt{\seqsplit{psbt.created}} in den Prüf- und Signierfluss übergeben und überspringt dabei den TX-Builder, da kein Aufbau der \ac{TX} mehr erforderlich ist.

Der manuelle Pfad besitzt gegenüber den externen Rails zwei bewusste Ausnahmen.
Zum einen entfällt der Abgleich der Zieladresse gegen die ext-Whitelist, sodass der Operator auch an nicht registrierte Adressen überweisen kann. 
Zum anderen überspringt \ac{OPA} für diesen Rail die Betrags- und Gebührengrenzen. Beide Ausnahmen sind erforderlich, damit der Operator im Ausnahmefall handlungsfähig bleibt. Als Restschranken bleiben bestehen: der Besitz der Operator-\ac{NATS}-Zugangsdaten, die Pflichtfeld- und Struktur-Prüfung samt \ac{SHA}256-Nachweis in \ac{OPA}, der Tages-Cap (\texttt{\seqsplit{max\_daily\_sats}}) (siehe \ref{sec:OPA_vel} sowie als harter, unabhängiger Backstop der Velocity-Cap des Signers (siehe \ref{sec:keyA_vel}. Der manuelle Pfad ist damit ein kontrollierter Bypass mit verbleibenden Schranken, kein ungeprüfter Kanal.
...
Der Signer führt in einem Redis-Speicher ein rollierendes Zeitfenster (Standard 24\,Stunden, \texttt{\seqsplit{VELOCITY\_WINDOW\_SEC}}) über den bereits signierten Abfluss. Dementsprechend weist dieser eine Anfrage mit \ac{HTTP}~429 ab, sobald die Summe den konfigurierten Grenzwert (Standard \num{35000000}~sats, \texttt{\seqsplit{VELOCITY\_CAP\_SATS}}) überschreiten würde.
...
Zudem wurde die Grenze von Key~A höher gewählt, sodass eine Hot to Cold \ac{TX} als Reaktion auf eine zuvor normal ausgeführte \ac{TX} ausgeführt werden kann und nicht die Schadens-verringerde Maßnahme des Basis-Systems verhindert. 
...
Sowohl die Nonce-Sperre als auch das Velocity-Fenster werden flüchtig in Redis gehalten und bewusst nicht persistiert. Der dauerhafte Schutz gegen das Wiedereinspielen vollständiger Signieraufträge ist die persistente \ac{PSBT}-\ac{ID}-Deduplizierung in der Datenbank. Nonce verengt lediglich das ohnehin kurze, über den Zeitstempel begrenzte Gültigkeitsfenster.
...
\paragraph{Möglcihkeit der Partner Authentifizierung} \mbox{}\\
XXX 
bei HTTP einbau von HMAC, basic auth oder sogar ouath für Partner kommunikation.
Sehr partner spezifisch und dementsprechend nciht aufgenommen. wird zum teil abgedeckt von whitelisting der wallet adressen
