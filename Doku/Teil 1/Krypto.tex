...
Die Schlüsselhalter-\acp{VM} werden über NixOS mit den in Tabelle \ref{tab:nixos_hardening} aufgeführten Maßnahmen deklarativ gehärtet.
\begin{table}[htbp]
\centering
\footnotesize
...
\begin{itemize}
  ...
  Ein möglicher gekoppelter Betrieb mit Bitcoin Core ermöglicht offline das Registrieren des Schlüsselmaterials in Bitcoin Core und eine GUI-Analyse mit Specter. Diese Kombination zeichnet sich jedoch durch eine signifikant größere Codebasis und damit eine erweiterten Angriffsfläche im Vergleich zu Sparrow aus. Zudem bleibt die Software in der \ac{PSBT}-Detailtiefe hinter Sparrow zurück und eignet sich ebenfalls als Teilservice nicht \cite{specter_devices, specter_faq_hotwallet, knowingbitcoin_coordinators}.
...
Reaktion. Für deren Zustellung wird ebenfalls dort beschriebene, selbst gehostete ntfy-Dienst mitgenutzt.
...
Multi-Signatur-Input (\texttt{cold_to_hot}-Nachbefüllungen) werden am Witness-Script herausgerechnet, während Wechselgeld-Rückgaben an die eigene Wallet über den Master-Fingerprint erkannt werden.
...
Velocity-Cap & unabhängiger Backstop, überlebt eine Kompromittierung des Basis-Systems & Signierer (Key~A \ac{VM}) & \texttt{VELOCITY\_CAP\_SATS}, \texttt{\
...
Physischer Zugriff auf Key~A \ac{VM} & Signierer & \ac{TPM}/\ac{PCR}-Bindung, LUKS & Diebstahl des Seed-Backups & \ref{sec:sicherheitsbetrachtung} \\
