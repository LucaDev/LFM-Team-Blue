...
\paragraph{Backup und Wiederherstellung} \ref{sec:backup} \mbox{}\\
...
Die Peer-Einrichtung und der HMAC-Austausch erfolgen über beiliegende Setup-Skripte auf beiden Systemen. Deren Verwendung ist in den READMEs der Repositories dokumentiert\cite{NixOsHot_github, Krypto_Basis_github}.
...
Für die Zustellung dieser Meldungen wird der bereits im Apphost-Kapitel beschriebene, selbst gehostete ntfy-Dienst mitgenutzt (siehe \ref{sec:apphost-monitoring} und \ref{sec:btc_db}).

Zudem werden Metriken über die Prometheus Integration und logs direkt über Loki ausgelesen und stehen dem Operator zur Verfügung. Die Daten werden d
...
Dies bedeutet, dass \acp{PSBT} mit Multi-Signatur-Input, also \texttt{\seqsplit{cold\_to\_hot}}-Nachbefüllungen und Wechselgeld Rückgaben an die eigene Wallet, erkannt durch Übereinstimmung des Witness-Script der \ac{PSBT} und mit dem Master-Fingerprint der eigenen Wallet, aus der Berechnung herausgezählt werden.

Die Beweggründe und Sicherheitsbetrachtung der gestaffelten Schranken und ihr Wertverhältnis wird weiter in der Sicherheitsbetrachtung diskutiert (siehe \ref{sec:doppelter_velocity}).
...
Um den in Abbildung \ref{fig4:cold_setup} dargestellten Prozess zu vereinfachen, stehen auf den Cold- und Hot-Wallet-\acp{VM} Hilf-Skripte zum Mounten, sicheren Auswerfen und Formatieren eines USB-Mediums auf dem Desktop bereit.
...
exponierte Basis-System, der gehärteten Signierer-\ac{VM} und der air-gapped Cold-Zone zwischen denen ausschließlich \acp{PSBT}, niemals Schlüsselmaterial, ausgetauscht werden.
