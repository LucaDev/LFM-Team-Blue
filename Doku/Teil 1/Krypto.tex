...
Verschiedene Parameter wurden für die Standard-Hot-Wallet-\ac{PSBT}-Erstellung festgelegt und können vom Mandanten in der Applikationslogik angepasst werden.  Diese können in Tabelle \ref{tab:psbt_params} nachvollzogen werden.
\begin{table}[h]
\centering
...
\hline
estimate\_mode & economical & Bevorzugt geringere Gebühren für langsamere Bestätigung. \\
\hline
...
Zudem wurde die Grenze von Key~A höher gewählt, sodass eine \texttt{hot\_to\_cold} \ac{TX} als Reaktion auf eine zuvor normal ausgeführte \ac{TX} ausgeführt werden kann und nicht die schadensverringernde Maßnahme des Basis-Systems verhindert. 
Ohne dies könnte ein Randfall eintreten, dass eine Hot \ac{TX} das tägliche Limit an seine Grenze bringt und dadurch normale \ac{TX} als auch \texttt{hot\_to\_cold} \ac{TX}, die vom Signierer nicht voneinander unterschieden werden können, abgeblockt werden. Durch den entwickelten Aufbau führt das Basis-System diese internen Wechsel-\ac{TX} weiter aus und das Key~A Limit wurde mindestens für die nächste \ac{TX} noch nicht ausgereizt.
...
Diese muss als neue Datei in Form ihres xpubs und Fingerprint auf das Wechselmedium kopiert werden und wie Key~A als watch-only Bestandteil auf der je anderen \ac{VM} eingetragen werden.
...
Neben der primär dargestellten Implementation kann der Mandant nach Übergabe des simulierten Systems entscheiden, zu anderen Ansätzen zu wechseln. Wie die Sparrow-Wallet Alternativen in \ref{sec:alt_sparrow} wurden verschiedene Ansätze evaluiert und werden in diesem Abschnitt diskutiert.
...
\subsubsection{Doppelter Velocity-Check}
...
Eine prägnante Darstellung dieses Sachverhalts kann in Tabelle~\ref{limits} gefunden werden.

\begin{table}[htbp]
\centering
\footnotesize
\begin{tabular}{@{}p{3.2cm} p{4.6cm} l l@{}}
\toprule
...
Tages-Abfluss (Daily-Cap) & deckelt den kumulierten Abfluss regulärer Zahlungen & \ac{OPA} (Basis-System) & \texttt{max\_daily\_sats} \\
Velocity-Cap & unabhängiger Backstop, überlebt eine Kompromittierung des Basis-Systems & Signer (Key-A-\ac{VM}) & \texttt{VELOCITY\_CAP\_SATS}, \texttt{\seqsplit{VELOCITY\_WINDOW\_SEC}} \\
...
\ac{HMAC}-Zeitfenster & begrenzt die Wiederverwendbarkeit abgefangener Signieraufträge & Signer (Key-A-\ac{VM}) & Signer-Konfiguration \\
\bottomrule
\end{tabular}
...
\centering
\footnotesize
\begin{tabular}{@{}p{3.4cm} l p{4.2cm} p{3.6cm} l@{}}
\toprule
...
Replay von Signieraufträgen & Signer & \ac{HMAC}, Nonce, \ac{ID}-Dedup & Redis-Reset & \ref{sec:redis_schwach} \\
Physischer Zugriff auf Key-A-\ac{VM} & Signer & \ac{TPM}/\ac{PCR}-Bindung, LUKS & Diebstahl des Seed-Backups & \ref{sec:sicherheitsbetrachtung} \\
