...
\texttt{\seqsplit{wgHMAC\_import.sh}} & Basis-System & Import des \ac{WG}-Peers von Key~A aus \path{/communication/wireguard.signer.json}; Hot-/Cold-Metadaten aus \path{/Wallets/hot/meta.json} und \path{/Wallets/Cold/}; \ac{HMAC}-Geheimnis aus \path{/communication/signer-hmac.secret}. \\
...
Es wird empfohlen, für die \acp{VM} von Key~B und Key~C zuerst die \ac{LAN}-Bridge von der \ac{VM} auf Hypervisor- oder Hardware-Ebene zu entfernen und anschließend Sparrow auf dem Desktop zu öffnen. Das Programm startet im Regtest-Modus, und die Variable \texttt{\seqsplit{cold.sparrowNetwork}} sollte in der Datei \path{sparrow.nix} bei produktiver Nutzung auf den Wert mainnet umgestellt werden. Zu beachten ist dabei außerdem, dass bei der Umstellung auf das Mainnet sämtliche Wallets neu aufgesetzt werden müssen (siehe \ref{sec:mainnet_migration}).
...
Velocity-Cap & unabhängiger Backstop, überlebt eine Kompromittierung des Basis-Systems & Signer (Key-A-\ac{VM}) & \texttt{VELOCITY\_CAP\_SATS}, \texttt{\
...
exponierte Basis-System, der gehärtete Signierer-\ac{VM} und der air-gapped Cold-Zone zwischen denen ausschließlich \acp{PSBT}, niemals Schlüsselmaterial, ausgetauscht werden.
