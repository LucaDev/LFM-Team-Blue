...
Installationsanleitung sowie auf den Anhang verwiesen, in dem die einze
...
Abschnitt~\ref{sec:apphost-herausforderungen}).
...
Im Rahmen des zweiten Aufgabenteils (\ac{KI}-Evaluation) sollten Agenten
...
Router verbleiben. An texttt{Cloudflare} fließen dabei keine Inhalte, sondern nur der
...
Abschnitt zu den Kompromissen). Auch Garage hat bewusst keinen
...
eines WireGuard-Zugangs werden im Abschnitt zu den eingegangenen Kompromissen
erläutert.

\subparagraph{Hintergrund} \mbox{}\\
...
den Bericht unter \texttt{/var/log/docker-security-scan.log} ab. Zusammen mit
...
\texttt{minimumReleaseAge} umgesetzt. Für Major-Updates beträgt sie 14~Tage
...
(Abschnitt~\ref{sec:warum-nixos}), stellt das einen Bruch dar. Dieselbe
...
\begin{itemize}
  \item \textbf{Keine offenen Ports.} Der Web-Zugriff erfolgt nur über das
    ...
    Auch intern wird zusätzlich lediglich \ac{SSH} für die Wartung benötigt.
...
\begin{itemize}
  ...
  \item \textbf{Keine fremde Cloud für Benachrichtigungen.}
    Push-Benachrichtigungen laufen, wie beschrieben, ausschließlich über das
    selbst gehostete ntfy.
\end{itemize}

\subsubsection{Eingegangene Kompromisse}

Die Grenzen eines Systems müssen offen benannt werden, sodass diese diskutiert und bewertet werden können.
Diese stellt ebenfalls den Entwicklungsprozess und besitzen hohe Relevant für diese Ausarbeitung.
Aufgrunddessen werden diese folgend thematisiert.
...
\begin{itemize}
  ...
  \item \textbf{\ac{OIDC} und der duale \ac{URL}-Raum.} Dienste mit \ac{OIDC}
    ...
    Mechanik dahinter wurde im Tor-Abschnitt erläutert. Es handelt sich um
...
\ac{AIDE} (Advanced Intrusion Detection Environment) prüft die Integrität des
...
\begin{itemize}
  ...
  \item \textbf{Updates.} Das System aktualisiert sich täglich über
    ...
    (Abschnitt~\ref{sec:apphost-digest-pinning}) und der Nix-Store wird
...
Abschließend wird dargelegt, an welchen Stellen größere Schwierigkeiten
auftraten und wie damit umgegangen wurde.

\begin{itemize}
  \item \textbf{Der komplette Umbau weg von Kubernetes.} Wie eingangs
    beschrieben, war der Stack zunächst auf Kubernetes mit Talos Linux
    aufgebaut und nahezu fertiggestellt. Die Erkenntnis, dass dieser Ansatz
    für den Anwendungsfall zu komplex ist, kam entsprechend spät. Die
    Entscheidung fiel dennoch für den vollständigen Neuaufbau auf Docker
    Compose, weil das Ergebnis dadurch wartbarer und für den Mandanten
    nachvollziehbarer wird. Im Rückblick war das die richtige Entscheidung,
    auch wenn sie erheblichen Mehraufwand bedeutete.
  \item \textbf{Architektonischer Bias.} Mitverantwortlich für die späte
    Kurskorrektur war eine über Jahre gewachsene Prägung. Durch langjährige
    Arbeit mit Kubernetes-Clustern sind Best Practices vertraut und fertige
    Lösungsbausteine oft vorhanden. Daraus entsteht die Tendenz, jede
    Herausforderung als Kubernetes-Anwendungsfall zu betrachten. Wichtig ist
    jedoch die objektive Einsicht, dass diese Herangehensweise in bestimmten
    Szenarien keinen messbaren Vorteil bietet oder sogar unnötige Komplexität
    erzeugt. Eines der Teammitglieder betreibt beispielsweise seit Jahren ein
    Kubernetes-basiertes Homelab (\url{https://github.com/LucaDev/Home/}).
  \item \textbf{NixOS-Versionssprung kurz vor Abgabe.} Gegen Ende der
    Bearbeitungszeit erschien die neue NixOS-Version. Der Sprung auf
    \texttt{26.05} brachte einige Breaking Changes mit sich. Ein Verbleib auf
    der älteren Version wäre einfacher gewesen. Dem Mandanten sollte jedoch
    kein von Beginn an veraltetes System ausgeliefert werden. Der
    Versionssprung wurde daher noch umgesetzt, was unter Zeitdruck
    zusätzlichen Aufwand bedeutete.
  \item \textbf{Kompatibilität von Kata Containers und gVisor.} Beide
    Sandbox-Runtimes sollten ursprünglich standardmäßig aktiv sein. In der
    Praxis verhielten sie sich je nach Proxmox-Konfiguration unterschiedlich,
    insbesondere wenn Nested Virtualization nicht stabil verfügbar war.
    Damit das Setup auf möglichst vielen Hosts problemlos startet, wurden
    beide Runtimes vorbereitet, aber nicht als Standard gesetzt. Sie lassen
    sich pro Dienst aktivieren, sobald die Hardware das stabil unterstützt.
  \item \textbf{Zeit und Vereinbarkeit.} Alle Teammitglieder arbeiten in
    Vollzeit und haben das Projekt neben weiteren Modulen bearbeitet.
    Zeitweise wurde zu viel Aufwand in Details investiert, vor allem in der
    Kubernetes-Phase. Die wichtigste Lehre daraus ist, frühzeitig zu
    hinterfragen, ob ein gewählter Ansatz wirklich zum Kern der Aufgabe
    passt, statt ihn aus Prinzip zu Ende zu führen.
  \item \textbf{Kata Containers und \texttt{userns-remap} (erst mit NixOS
    26.05 aufgetreten).} Nach dem Versionssprung startete
    \texttt{docker.service} nicht mehr. \texttt{dockerd} verweigerte den
    Start mit der Meldung \textit{"containerd-snapshotter is explicitly
    enabled, but is not compatible with userns remapping"}. Kata Containers
    3.x liefert keine klassische OCI-Runtime-Binary mehr mit, sondern wird
    ausschließlich über den containerd-v2-Shim
    (\texttt{containerd-shim-kata-v2}) angesprochen. Genau das erfordert
    Dockers containerd-Snapshotter-Integration, die wiederum mit
    \texttt{userns-remap} inkompatibel ist. Da das UID/GID-Remapping
    wichtiger erschien als die direkte Ansprache von Kata über den
    Docker-Daemon, wurde der \texttt{kata-runtime}-Eintrag aus der
    Docker-Konfiguration entfernt und \texttt{containerd-snapshotter}
    deaktiviert. Kata Containers bleiben über
    \texttt{virtualisation.containerd} (CRI) weiterhin konfiguriert und
    nutzbar, nur nicht mehr direkt via
    \texttt{docker run --runtime=kata-runtime}.
  \item \textbf{AppArmor blockiert Audit-Regeln.} Mit gleichzeitig aktivem
    \texttt{security.audit.enable} und \texttt{security.apparmor.enable}
    scheiterte \texttt{audit-rules-nixos.service} bei jedem
    \texttt{nixos-rebuild switch} mit \textit{"There was an error in line 2
    [...] audit.rules"}, unabhängig vom Inhalt der Regeln. Ursache war ein
    Fehler in der Ladereihenfolge der Linux-Security-Module bei aktiviertem
    AppArmor (\href{https://github.com/NixOS/nixpkgs/issues/483085}{nixpkgs
    Issue \#483085}), behoben in
    \href{https://github.com/NixOS/nixpkgs/pull/535329}{nixpkgs PR
    \#535329}. Erschwerend kam hinzu, dass der Upstream-Fix erst wenige Tage
    vor Abgabe verfügbar war.
  \item \textbf{Lange Wartezeiten für die Erprobung von Neuinstallationen.}
    Der umfangreiche Setup-Prozess umfasst das Zurücksetzen der \ac{VM}, das
    Booten der LiveCD, das Klonen des Repositorys, die NixOS-Installation,
    die Secure-Boot-Konfiguration und den Start der Container. Das Testen von
    Änderungen am Installationsprozess war dadurch mitunter langwierig und
    zeitintensiv.
  \item \textbf{Traefik und Umgebungsvariablen.} Traefik ignoriert die Werte
    \texttt{\$\{ACME\_EMAIL\}} und \texttt{\$\{DOMAIN\}} aus der
    \texttt{.env}, denn es kann Werte in der statischen Konfiguration nicht
    durch Umgebungsvariablen ersetzen. Für eine saubere und einfache
    Installation musste die \texttt{traefik.yaml} daher templatisiert
    werden.
\end{itemize}