...
\toprule
\textbf{Härtungs-}\newline \textbf{maßnahme} & \textbf{Beschreibung} & \textbf{Abgewehrter Angriffsvektor} \\
\midrule
...
\addlinespace
Kernel-Härtung
  & \texttt{kptr\_restrict}, \texttt{dmesg\_restrict}, \texttt{yama.ptrace\_scope}, deaktiviertes unprivilegiertes \acs{eBPF} sowie \texttt{protectKernelImage} (kein kexec zur Laufzeit).
...
\bottomrule
\end{tabularx}
\caption{Deklarative NixOS-Härtung der Schlüsselhalter-\acs{VM}s}
...
\multicolumn{3}{@{}l}{\textit{Basis-System (Hot-Wallet, \path{scripts/hotwallet/ops/})}}\\
psbt\_submit.sh                 & Basis & Fertige Operator-\ac{PSBT} über \ac{NATS} (\texttt{psbt.submit.requested}) einreichen. \\
...
\addlinespace
\multicolumn{3}{@{}l}{\textit{Cold-\acp{VM} (Key~B/C, Desktop, \path{scripts/})}}\\
\addlinespace
