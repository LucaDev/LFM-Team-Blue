...
Einzig die Middleware ist zusätzlich an zwei weitere Netzwerke angebunden und schickt Daten an das \texttt{communication-network}, um Alarme über den bestehenden ntfy-Dienst zuzustellen. Es empfängt Zahlungsanfragen an seiner \ac{API} unter einer eigenen Subdomain des reverse Proxy von dem \texttt{traefik-public}-Netzwerk.
Authelia-Anmeldung wird an diesem Router bewusst verzichtet, da die \ac{API} von Partner-Systemen maschinell aufgerufen wird und ein Browser-Login-Redirect diese Integration brechen würde. die Autorisierung
erfolgt stattdessen, wie beschrieben, über das Wallet-Whitelisting und die
\ac{OPA}-Policies. Die Verbindung zur Signierer-\ac{VM} verläuft davon
getrennt ausschließlich über den im Folgenden beschriebenen
WireGuard-Tunnel.
