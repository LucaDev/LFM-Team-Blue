...
Die Kategorien folgen damit derselben Logik wie das gesondert operationalisierte Bitcoin-Konzept (Abschnitt~\ref{subsec:btc-bewertung}). Dabei werden die Unterkriterien lediglich zusätzlich gewichtet, weil sie sich in ihrer Sicherheitsrelevanz deutlich stärker unterscheiden.
...
\hline
\end{tabular}
\caption{Prüfindikatoren der Hauptkategorien für die Basis-System Komponente}
...
Kernanforderung des Mandanten, Zahlungen tatsächlich als Bitcoin-Transaktion auszuführen.
...
Zudem verwendet dieser Ansatz \path{nftables} und \ac{iptables} als Sicherheitsmodelle, was zeigt, dass diese Sicherheitsaspekte evaluiert wurden, sich jedoch nicht für einen Ansatz entschieden wurde. Beide zu verwenden wird als schlechte Praxis angesehen und sollte vermieden werden.
...
\paragraph{Architekturplanung (2 von 3)}
I1 (2): Die Punktzahl begründet sich daraus, dass ein Rollen- und Zonenmodell ist vorhanden.
Dabei existieren vier getrennte LXC-Rollen bestehend aus (\texttt{201 tor-edge}, \texttt{202 app-core}, \texttt{203 ops} und \texttt{204 btc-node}), welche sich die zwei bridges \texttt{vmbr1} für Service- und \texttt{vmbr2} für Monitoring-Verkehr, definiert über \path{/etc/network/interfaces.d/homelab-bridges.cfg}, teilen. 
Explizite
...
I2 (2): Die Platzierung ist grundsätzlich begründet (Host bleibt frei von Workloads, Bitcoin als eigene Rolle). 
Der Abzug ergibt sich daraus, dass \texttt{202} und \texttt{204} dual-homed an beiden Bridges 
(\texttt{10.10.10.10}/\texttt{10.10.20.10} beziehungsweise
...
I4 (2): Der Bitcoin-Bereich ist sauber getrennt mit eigener Rolle, RPC und P2P nur
...
HTTP-Statuscheck validiert, etwa Vaultwarden mit 200, Paperless-ngx mit 302 und
Stirling-PDF mit 401.\cite{runbookA,composeA}
I2 (1): Elf Dienste unterschiedlichen Schutzbedarfs wurden auf einer einzigen
Rolle gebündelt, was sich als problematisch herausstellt. Der Passwortmanager Vaultwarden teilt sich Container und Postgres-/Redis-Unterbau mit Alltagsdiensten wie Stirling-PDF und
...
\paragraph{Konfiguration (2 von 3)}
...
I2 (1): Sämtliche Secrets werden im Klartext unter \path{/root/homelab-secrets} erzeugt und in \path{service-credentials.txt} inklusive Benutzer/Passwort-Paaren aggregiert. 
...
\paragraph{Netzwerk, Tor und Zugriff (2 von 3)}
...
Containerseitig endet die \path{HOMELAB-DOCKER-ACCESS}-Kette zwar mit \texttt{DROP}, hängt aber nur in \path{DOCKER-USER} und schützt damit ausschließlich Docker-publizierte Ports.
CT-eigene Dienste wie \texttt{sshd} oder \texttt{node-exporter} werden von diesen policies nicht addressiert.\cite{hostnftablesA,dockerfirewallA,selfauditA}
...
I3 (1): Die Admin-Oberflächen Grafana und Uptime Kuma sowie der Alerts-Feed wurden als normale Onions ohne v3-Client-Auth publiziert. Parallel blieb Host-SSH auf \texttt{permitrootlogin yes} und \texttt{passwordauthentication yes}.\cite{runbookA,selfauditA}
...
\paragraph{Backup und Monitoring (1 von 3)}
...
\paragraph{Fehleranalyse (3 von 3)}
...
I2 (3): Die Befunde sind fachlich eingeordnet mti einer Klassifizierung der Mitsicherung von
\path{var/lib/tor} als Hidden-Service-Key-Leak, der RPC-Zugang über eine Onion-Adresse als Remote-Signierfläche und \texttt{policy accept} als Bruch des Default-Deny-Prinzips.
...
und 8006 begrenzt.
\path{docker-compose.fixed.yml} entfernt \texttt{privileged} und den Docker-Socket, bindet alle Ports auf \texttt{127.0.0.1} und erzwingt Secretssowie Image-Tags über Fail-fast-Variablen (\texttt{\$\{VAR:?\}}).\cite{artefaktpruefungA}
...
Abgesichert soll dieses System über \path{nftables}, wobei dies einen validen Ansatz darstellt
...
\paragraph{Architekturplanung (3 von 3)}
...
I2 (3): Die Provisionierungsmatrix begründet je Gast Typ, Zone, Bridge und Isolationsprofil. Dabei führt hoher Schutzbedarf zu \acp{VM} aus einem gepinnten Debian-Cloud-Image, Infrastrukturrollen zu unprivilegierten LXCs ohne \texttt{nesting}, \texttt{keyctl}, \texttt{fuse} oder \texttt{mknod}.
...
I4 (3): Admin-, Anwendungs- und Bitcoin-Bereich sind strikt getrennt. Der Admin-Pfad führt ausschließlich über Tor mit v3-Client-Auth auf den Admin-Onion beziehungsweise für die Proxmox-Web-UI über einen lokalen SSH-Tunnel.
Bitcoin bildet eine eigene Dummy-only-Zone auf \texttt{vmbr50} mit eigener Verbotsliste.\cite{masterplanB,finalsummaryB}

\paragraph{Services (1 von 3)}
...
\paragraph{Konfiguration (3 von 3)}
I1 (3): Alle Gäste erhielten über \path{section3_config_remote.sh} eine reproduzierbare gehärtete Minimalbasis. Diese stellt echte OS-Updates (\texttt{--with-new-pkgs}), \texttt{Etc/UTC}, Journald-Limit \path{SystemMaxUse=64M}, die \path{/etc/ailab}-Struktur mit 0700-Secrets-Verzeichnis und \path{policy-rc.d} gegen ungewollte Dienststarts bereit. 
Die VM-Basis wurde gegen \texttt{SHA512SUMS} samt Signaturdatei verifiziert.\cite{section3scriptB,validatornotesB,masterplanB}
I2 (3): In IaC- und Doku-Pfaden liegen keine Klartext-Secrets. Dabei enthält \path{/etc/ailab/secrets} nur ein README mit dem expliziten Verbot echter Geheimnisse. 
Laufzeitnahe Artefakte wie der Operator-Auth-Key liegen root-only unter \path{/root/ailab-runtime} außerhalb von \texttt{outputs} und IaC.\cite{section3scriptB,implementationlogB}
I3 (3): Jeder Abschnitt endet mit einem Validator-Lauf, der Datei- und Live-Zustand abgleicht (Paketmanifest und Port-Check je Gast). Abweichungen wie die DHCP-Instabilität von \texttt{101} sind samt Korrektur dokumentiert.\cite{validatornotesB,implementationlogB}
I4 (3): Reproduzierbarkeit ist über durchgängige Snapshots wie beispielsweise \texttt{post-provision-base}, \texttt{post-config-base} und \texttt{post-network-tor-base} für alle acht Gäste gegeben. Zudem belegen definierte Soll-Zustände und der nachgewiese Rückbau des Temp-Pfads
diesen Zustand.\cite{finalsummaryB,validatornotesB}

\paragraph{Netzwerk, Tor und Zugriff (3 von 3)}
I1 (3): Die Tabelle \texttt{inet ailab} setzt \texttt{policy drop} auf \texttt{input} und \texttt{forward}. Dadurch werden werden nur Loopback, \texttt{established/related} sowie zwei quellgebundene Pfade (\texttt{vmbr0} mit \texttt{ip saddr @operator\_v4} auf die Ports 22 und 8006
sowie \texttt{vmbr10} mit der Adresse des Tor-Relays auf Port 22) akzeptiert.
Da \texttt{pve-firewall} deaktiviert wurde, ist genau ein Regelwerk wirksam.\cite{section4scriptB,finalsummaryB}
...
I3 (3): Der Admin-Onion ist auf genau einen autorisierten Client verengt, sodass service-seitig eine einzige Datei \path{operator-1.auth} (Rechte 0600, \texttt{debian-tor}) unter \path{authorized_clients} und host-seitig ein einziges root-only \path{operator-1.auth_private} außerhalb des IaC-Pfads existieren.
...
\paragraph{Backup und Monitoring (2 von 3)}
I1 (1): Das Borg-Repository ist mit \texttt{repokey-blake2} verschlüsselt und der Zugriff über den dedizierten Nutzer \texttt{borgrepo} mit \texttt{restrict}, \texttt{ForceCommand borg serve --restrict-to-path}, \texttt{PermitTTY no} und deaktiviertem Forwarding verengt.
...
I2 (2): Der Restore wurde als Smoke-Restore des Archivs \path{ailab2-20260707T212412Z} mit  \path{host/configs.tgz}, \path{ct-101/sanitized-configs.tgz}, \path{EXCLUSIONS.txt}) nachgewiesen.
Die Wiederherstellung einer vollständigen Rolle fehlt.\cite{finalsummaryB,validatornotesB}
I3 (2): Der Host wird erstmals mitüberwacht (\texttt{node-exporter} auf \texttt{10.30.30.1:9100} plus SSH-Auth-Textfile-Metriken), wobei alle sechs Prometheus-Targets \texttt{health=up} melden. 
...
\paragraph{Fehleranalyse (3 von 3)}
I1 (3): Alle vier Artefakte wurden mit tabellarischen Fundlisten (Priorität, Typ, Stelle) analysiert, darunter Befunde über die Analyse aus Szenario A
hinaus, etwa dass \texttt{table inet} die Freigaben auch für IPv6 wirksam
macht und dass direkt in finale Archivdateien geschrieben
wird.\cite{artefaktreviewB}
...
I3 (3): Die Referenzfassungen beheben die Kernprobleme, sodass die \path{firewall.fixed.nft} \texttt{policy drop} nutzt, die \texttt{define}-Variablen und Sets den Admin-Zugriffe an die Quelladresse \texttt{10.0.2.2} bindet und den Egress auf DNS, NTP und HTTP(S) begrenzt.
Die Datei \path{backup.fixed.sh} ergänzt \texttt{age}-Verschlüsselung, Generationen-Verzeichnisse und eine \path{EXCLUSIONS.txt}.\cite{artefaktreviewB}
...
Die Trennung von Schlüsselmaterial und Rollen erhält 1 von 3 Punkten. Auf Dateiebene wurde die Trennung technisch erzwungen (siehe XXXX), jdoch bleibt der Gegenstand dieser
Trennung hinter dem Kriterium zurück. Die Watch-only- und Hot-Service-Rollen wurden nicht als Dienste implementiert, sondern existieren nur als Verzeichnis- und Datei-Choreografie mit Dummy-Artefakten. Ein Service für die Rollenfunktionen wurde nicht gebaut. Damit existierte zu keinem
...
Zweitens existiert für die eigentlichen Bitcoin-Kernfunktionen keine praktische Prüfbarkeit. Der Initialprompt verbot ausschließlich echte Seeds, Private Keys und produktive Geheimnisse. Eine lokale Regtest-Umgebung erzeugt lediglich wertloses Testschlüsselmaterial über einen eigenen Ableitungspfad und hätte diesen Rahmen somit nicht verletzt.
...
In den nächsten Abschnitten folgt eine Detailanalyse des Vergleiches. Szenario A und B werden dabei sowohl miteinander als auch mit der eigenen Ausarbeitung verglichen.
...
Szenario B erhielt hier 1 von 3 Punkten. Zwar orientierte sich die geplante Rollenverteilung konsequent am Schutzbedarf und an der Zonentrennung, der Ziel-Servicekatalog verblieb jedoch vollständig auf Planungsebene. Es zeigt sich die Kehrseite einer engen Steuerung, sodass das Modell die vorgegebenen Sicherheits- und Nachweisziele so stark priorisierte, dass
die eigentliche fachliche Kernleistung des Servicekatalogs nicht mehr
umgesetzt wurde.\cite{masterplanB,finalsummaryB}
...
Szenario B erhielt hier 2 von 3 Punkten. Positiv sind der Borg-basierte Backup-Pfad mit \texttt{repokey-blake2}-Verschlüsselung und \texttt{--restrict-to-path}-verengtem Repository-Zugang, ein durchgeführter Smoke-Restore, die bewusste Aussonderung der Hidden-Service-Schlüssel aus dem Backup sowie die operator-only Monitoring-Basis mit Host-Exporter. 
Für die volle Punktzahl fehlen drei Nachweise: Das Backupziel liegt weiterhin auf
demselben Testhost, was die Dokumentation selbst als offenes Architektur-Risiko ausweist, und die Borg-Passphrase liegt im Klartext auf ebendiesem Host. Der Restore wurde nur für drei Einzeldateien, nicht für eine vollständige Rolle nachgewiesen. Eine Alarmzustellung bis zum Betreiber wurde nicht durchgetestet. Damit erfüllt auch Szenario B den Indikator \glqq nicht
ausschließlich lokal\grqq{} nicht, wenngleich die Sicherung im Unterschied zu Szenario A erschlüsselt und der Zugriffspfad technisch verengt
ist.\cite{finalsummaryB,validatornotesB}

\subsection{Bitcoin-Konzept}
Nach der gewichteten Detailbewertung (siehe Tabelle~\ref{tab:btc-detailbewertung}) erreichte Szenario A 1{,}45 von 3{,}00 Punkten (gerundet 1), Szenario B 1{,}75 Punkte (gerundet 2) und die Eigenentwicklung aus Kapitel~\ref{Krypto} 2{,}85 Punkte (gerundet 3). 
...
Die Rundung auf ganze Punkte überzeichnet dabei den Abstand der beiden \ac{KI}-Szenarien. Ungerundet trennen Szenario A und B lediglich 0{,}30 Punkte, während die gerundete Darstellung einen vollen Punkt suggeriert. Für die Interpretation sind deshalb die ungerundeten Werte aus Tabelle~\ref{tab:btc-detailbewertung} maßgeblich.

Auf die Details der Eigenentwicklung wird nur grob eingegangen und sind deswegen nur kurz in der Tabelle~\ref{tab:btc-eigen-details} aufgelistet. Diese Tabelle begründet die gegebene Bewertung und liefert Vergleichspunkte für die nächsten Absätze. Die genannten Aspekte können in der vorherigen Ausarbeitung nachgelesen werden \ref{Krypto}.
...
Drei Vertrauenszonen aus exponiertem Basis-System, WireGuard-isolierter Signierer-\ac{VM} und air-gapped Cold-Zone; explizite \ac{BIP}-174-Rollenzuordnung; dokumentierte Abwägung verworfener Alternativen wie Online-Koordinator und Hash-Freigabeschicht. \\
\addlinespace
Trennung Schlüsselmaterial / Rollen & 3 &
Basis-System nur mit Watch-only-Deskriptoren, Key~A \ac{TPM}-versiegelt hinter WireGuard, Key~B und Key~C air-gapped; Segregation of Duties über reale, per Regtest-Ableitungspfade erzeugte Schlüssel. \\
\addlinespace
PSBT-, Signier- \& Handoff-Prozess & 3 &
...
Container mit \path{cap_drop: [ALL]}, \path{read_only}, \path{no-new-privileges}, non-root-\ac{UID} sowie \path{mem_limit} und \path{pids_limit}; \path{nftables} mit Drop auf Ein- und Ausgang und WireGuard-only; \ac{HMAC} mit Nonce und Deduplizierung; zweistufiger Velocity-Cap; NixOS mit Flake-Pinning. \\
\addlinespace
...
\bottomrule
\end{tabularx}
\caption{Detailbewertung der Eigenentwicklung im Bitcoin-Konzept}
\label{tab:btc-eigen-details}
\end{table}

\subsubsection{Architekturschlüssigkeit}
\textbf{Eigenentwicklung: 3, A: 2, B: 2.} Die Eigenentwicklung zeichnet sich durch das Entwickeln und Verwerfen verschiedener Ansätze dar und bildet einen durchgehenden finalen Ansatz.
...
Außerdem zeichnen sich die beiden \ac{KI}-Szenarien durch nur sehr oberflächliche Überlegung aus. Die Eigenentwicklungen unterstützt die manuelle Nutzer-Verifikation von \acp{PSBT} in der Cold-Wallet durch die automatische Signierung durch Key~A, was dem Operator direkt in der \ac{GUI} angezeigt wird. 
Eine vergleichbare tiefe Überlegung oder Unterstützung des Operator such man in den \ac{KI}-Szenarien vergebens.

\subsubsection{Trennung von Schlüsselmaterial und Rollen}
\textbf{Eigenentwicklung: 3, A: 1, B: 2.} In der Eigenentwicklung ist die Trennung dreifach technisch erzwungen und ein aufwändiges Modell für die Segregation of Duties aufgebaut worden.
...
Szenario A diese nicht vollständig und relevante Lücken aufweist. Bitcoin Core wird zwar als Node betrieben, jedoch nicht in einem Testmodus, sodass der Node über den Start des Blockchain-Scannings nicht hinauskommt. 
...
Szenario B führt eine vollständige Prozess-Choreografie durch, deren Artefakte jedoch aus JSON-Metadaten statt \acp{PSBT} bestehen und deren Signatur nur über eine Markerzeichenkette simuliert wird. Zwar werden einzelne Aspekte evaluiert und etwa Bitcoin Core ausgewählt, umgesetzt wird jedoch keine der Maßnahmen. Die Ansätze verlassen die theoretische Phase nicht und der Prompter wird stattdessen mit statischen Ausgaben getäuscht.

Beide \ac{KI}-Szenarien scheitern damit am selben Kernkriterium aus entgegengesetzten Richtungen. A liefert den Unterbau ohne Prozess und B den Prozess ohne Kryptographie. Für den Mandanten bedeutet dies, dass keines der beiden \ac{KI}-Szenarien die eigentliche Kernanforderung erfüllt, eine Zahlung tatsächlich als Bitcoin-Transaktion auszuführen. 
...
\textbf{Eigenentwicklung: 2, A: 2, B: 1.} Die Eigenentwicklung stellt mit der Regtest-Umgebung genau den Validierungsansatz bereit, den die Metrik als Maßstab nennt. Ergänzt wird dies um Simulationsskripte für Spend-, Refill- und Sicherungspfad jedoch werden tatsächliche Laufzeit-Tests vermisst. Dieser Ansatz eines Testens des \ac{BTC}-Netzwerk mit einer realitätsnahen Simulation wird in A und B wird nicht umgesetzt. Während A noch regtest evaluiert, wird der mögliche Einsatz nicht von Szenario B erkannt. Stattdessen simuliert B einen eigenen, sehr primitiv, statischen Prozess und bleibt damit weit hinter A zurück. 
Szenario A prüfte seine Node-Schicht mit einem echten Laufzeit-Prüfskript, kam jedoch nicht über das scanning der mainnet Blockchain hinaus. Dementsprechend stellt sich A durch einen sehr viel kleineren Umfang, aber dafür mit einem adäquateres Testing dar und erreicht so eine gleiche Wertung.
...
Ein weiterer Unterschied liegt in der Beobachtbarkeit des Bitcoin-Systems selbst. 
...
Eine verbleibende Lücke betrifft auch hier die Lieferkette der Eigenentwicklung. Der Container-Build der Hot-Wallet klont das offizielle Bitcoin-Core-Repository und checkt lediglich das Tag \path{v31.0} aus, ohne den zugehörigen Commit-Hash zu pinnen oder das Tag kryptographisch zu verifizieren. Das ist dieselbe Fehlerklasse wie die fehlende Signaturprüfung in Szenario A, wenn auch mit kleinerer Angriffsfläche, da direkt aus dem Quell-Repository gebaut wird. 
...
Bemerkenswert ist, dass allein die Eigenentwicklung ein Konzept für den Verlust von Schlüsselmaterial besitzt, also den für ein Bitcoin-System kritischsten Fehlerfall.
