...
Auf die Details der Eigenentwicklung wird nur grob eingegangen und sind deswegen nur kurz in der Tabelle~\ref{chap:ki-evaluation} aufgelistet. Diese Tabelle begründet die gegebene Bewertung und liefert Vergleichspunkte für die nächsten Absätze. Die genannten Aspekte können in der vorherigen Ausarbeitung nachgelesen werden \ref{Krypto}.
...
Außerdem zeichnen sich die beiden \ac{KI}-Szenarien durch nur sehr obeflächliche Überlegung aus. Die Eigenentwicklungen unterstützt die manuelle Nutzer-Verifikation von \acp{PSBT} in der Cold-Wallet durch die automatische Signierung durch Key~A, was dem Operator direkt in der \ac{GUI} angezeigt wird. 
...
A stellt sich im Verlgeich zu B als sehr viel kleiner, aber dafür ehrlicher heraus.
...
Szenario A härtet seinen Node solide über systemd-Sandboxing wie \path{ProtectSystem=full} und \path{NoNewPrivileges}, betreibt ihn aber nicht containerisiert und lässt zudem die Signaturprüfung der Bitcoin-Core-Installation aus, also den sicherheitskritischsten Schritt der Lieferkette. Die Mischung aus \path{nftables} und \path{iptables} (siehe paht{homelab-btc-firewall.sh}) fällt wie bereits erwähnt ebenfalls negativ auf.
Daneben verwendet das Szenario für die \ac{BTC}-Core Authentifizierung direkt das Cookie Modell (siehe path{bitcoin.conf}: \emph{rpccookiefile=/var/lib/bitcoind/.cookie}), was sich als eine gute Härtung darstellt und die unauthorisierte \ac{PSBT} Erstellung für registrierte Wallets durch einen Angreifer weiter erschwert.
