...
Die wertungsbestimmenden Einzelbefunde sind:
\begin{itemize}
\item \textbf{Architekturplanung, I3 (1):} Erlaubte Kommunikationspfade wurden nicht als dokumentiertes Regelwerk festgelegt und der Ost-West-Verkehr zwischen den Containern blieb weitgehend offen.\cite{selfauditA}
\item \textbf{Services, I1 (3) und I2 (1):} 20 Container-Services waren lauffähig und validiert, jedoch wurden elf davon auf einer einzigen Rolle (\texttt{app-core}) gebündelt statt nach Schutzbedarf verteilt.\cite{runbookA}
\item \textbf{Konfiguration, I2 (1):} Zahlreiche Secrets lagen konzentriert im
Klartext unter \path{/root/homelab-secrets} und werden zusätzlich durch den Backup-Job die Hidden-Service-Secret-Keys unverschlüsselt in ein lokales Archiv exportiert.\cite{selfauditA,backupscriptA}
\item \textbf{Netzwerk, I1 (1) und I3 (1):} Ein durchgehendes Default-Deny war
nicht nachweisbar. Im Detail besaß der Host zunächst keine Input-Filter, und die
Container-Firewalls schützen nur die \texttt{DOCKER-USER}-Chain. Die publizierten Admin-Onions für Grafana und Uptime Kuma besaßen keine zusätzliche Zugriffsbeschränkung wie v3-Client-Auth.\cite{selfauditA,runbookA}
\item \textbf{Backup und Monitoring, I1 (0), I2 (0), I4 (1):} Die Routine-Backups waren unverschlüsselt und lagen ausschließlich lokal auf demselben Host. Dabei fank kein Restore-Test statt und eine Alarmzustellung bis zum Betreiber war nicht nachweisbar.\cite{selfauditA,backupscriptA}
\end{itemize}
...
Die wertungsbestimmenden Einzelbefunde lauten:
\begin{itemize}
\item \textbf{Services, I1 (0):} Der geforderte Servicekatalog von mindestens 15 Diensten verblieb vollständig auf Planungsebene, da bis zum Ende des Runs keine App-Rollouts erfolgten.
Als Zusatzpakete wurden ausschließlich \texttt{qemu-guest-agent} und \texttt{cloud-guest-utils} installiert. Fünf der acht Gäste verblieben im Sollzustand \texttt{stopped}. 
Dementsprechend verbleiben I2 und I3 bei einem Punkt, da Verteilung und Rechtemodell nur für die dienstlose Minimalbasis nachweisbar sind.\cite{masterplanB,finalsummaryB}
\item \textbf{Netzwerk, I2 (2):} Mangels ausgerollter Dienste existiert nur der Admin-Onion als nachweisbarer Tor-Pfad, sodass allgemeine Service-Onions nicht eingerichtet wurden. Dadurch ist nur der Indikator für den Admin-Pfad belegt.
ist.\cite{finalsummaryB}
\item \textbf{Backup und Monitoring, I1 (1):} Das Borg-Repository wird mit \texttt{repokey-blake2} verschlüsselt, liegt aber ausschließlich auf demselben Testhost, was die Dokumentation selbst als offenes Architektur-Risiko ausweist. Die Passphrase selbst liegt im Klartext auf dem gesicherten Host.  
I2 (2): Der Restore wurde nur als Smoke-Restore dreier Einzeldateien nachgewiesen,
nicht als Wiederherstellung einer Rolle. 
I4 (2): \texttt{ntfy} ist nur intern erreichbar, eine Zustellung bis zum Betreiber wurde nicht
durchgetestet.\cite{finalsummaryB,validatornotesB}
\end{itemize}
