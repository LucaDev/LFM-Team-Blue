...
\hline
\textbf{Gesamtbewertung} & & \textbf{1,45} & & \textbf{1,5} & & \textbf{2,85} \\
\hline
\end{tabular}
...
\paragraph{Fehleranalyse (3 von 3)}
I1 (3): Alle vier Artefakte wurden mit tabellarischen Fundlisten (Priorität, Typ, Stelle) analysiert. Darunter finden sich auch strukturelle Befunde wie die IPv6-Wirkung der \texttt{table inet}-Freigaben und das direkte Schreiben in finale Archivdateien.\cite{artefaktreviewB}
...
Auf die Details der Eigenentwicklung wird analog zum Bitcoin-Konzept nur kurz eingegangen. Tabelle~\ref{tab:basis-eigen-details} begründet die vergebenen Wertungen und liefert die Vergleichspunkte für die folgenden Absätze. Die genannten Aspekte können in der Ausarbeitung des Basis-Systems nachgelesen
...
\toprule
\textbf{Kategorie} & \textbf{Score} & \textbf{Details der Eigenentwicklung} \\
\midrule
Architekturplanung & 3 &
Dokumentierte Erprobung und begründete Verwerfung eines
Kubernetes-Ansatzes zugunsten eines einzelnen gehärteten Apphosts.
Getrennte Docker-Netzsegmente je Dienstgruppe. Nachgewiesene Abdeckung des
vorgegebenen Bedrohungsmodells je Angriffsvektor. \\
\addlinespace
Services & 3 &
Über 18 produktive Dienste aus Medien, Dokumenten, Cloud, Hausautomation
und Entwicklung. Datenbanken und Caches bleiben ohne Veröffentlichung nur im
jeweiligen Netz erreichbar. Zentrale Anmeldung über Authelia mit \ac{OIDC}
und Forward-Auth. \\
\addlinespace
Konfiguration & 3 &
Vollständig deklarativer Systemzustand über NixOS-Module und Compose-Dateien
im Git. Atomare Updates mit Rollback-Generationen. Secrets außerhalb des
Repositories mit abgeleiteten Argon2id-Hashes. Kontrollierte Updates über
RenovateBot und wöchentliche Trivy-Scans. \\
\addlinespace
Netzwerk / Tor / Zugriff & 3 &
Kein nach außen geöffneter Port dank \ac{DNS}-01-Zertifikaten. Default-Deny
über nftables. Traefik als einziger Web-Einstieg mit \ac{TLS} nach
\ac{BSI} \ac{TR}-02102-2 und zentralen Security-Headern. Tor-Erreichbarkeit
pro Dienst explizit zuschaltbar. Mehr-Faktor-Anmeldung und Fail2ban. \\
\addlinespace
Backup / Monitoring & 2 &
Konsistente Proxmox-Snapshots des Gesamtzustands mit optionaler
Verschlüsselung. Vollständiges Monitoring von Host und Containern.
Alarmkette über selbst gehostetes ntfy bis auf das GrapheneOS-Endgerät.
Kein dokumentierter Restore-Test und kein umgesetztes Off-Host-Ziel. \\
\midrule
\textbf{Gesamt} & \textbf{14 / 15} & Basis-Kategorien ohne Fehleranalyse \\
\bottomrule
\end{tabularx}
\caption{Detailbewertung der Eigenentwicklung in den Basis-Kategorien}
\label{tab:basis-eigen-details}
\end{table}
...
Szenario B belegt seine Strenge dagegen nur für den einzigen vorhandenen Admin-Pfad. 
...
Nach der gewichteten Detailbewertung (siehe Tabelle~\ref{tab:btc-detailbewertung}) erreichte Szenario A 1{,}45 von 3{,}00 Punkten (gerundet 1), Szenario B 1{,}5 Punkte (gerundet 2) und die Eigenentwicklung aus Kapitel~\ref{Krypto} 2{,}85 Punkte (gerundet 3). 
...
Innerhalb der vollen Punktzahl arbeitete Szenario B gleichwohl tiefer als Szenario A. Sein Review fand Befunde, die A nicht benannte, etwa die IPv6-Wirkung der \texttt{inet}-Tabelle und das direkte Schreiben in finale Archivdateien, und wog für jede Datei Korrekturvarianten systematisch gegeneinander ab. 
