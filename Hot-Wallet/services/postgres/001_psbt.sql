...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'psbt_state') THEN
    CREATE TYPE btc.psbt_state AS ENUM ('INTENT_CREATED', 'PSBT_CREATED', 'PSBT_FAILED', 'OPA_APPROVED', 'OPA_REJECTED', 'WAITING_HUMAN', 'COLD_STARTED', 'COLD_STOPPED', 'WAITING_RETRY', 'SIGNING_FAILED', 'SIGNED', 'FINALIZED', 'FINALIZE_FAILED', 'BROADCASTED', 'BROADCAST_FAILED');
  END IF;
