...
| `manual`| NATS `psbt.submit.requested` (Operator-Identität)| Bypass | übersprungen | ja |
| `OPA_hot`/ `OPA_cold`| interne Refill-/Sicherungs-Logik | interne Wallets | übersprungen (systemgewählt) | – |

Der **Operator-Pfad** (`manual`) erlaubt es, eine eigene Hot-Transaktion auszulösen – entweder als „Betrag + Adresse" (Intent über die NATS-`operator`-Identität) oder aals fertige PSBT über das NATS-Subject psbt.submit.requested unter der Operator-Identität. Der Whitelist-Bypass ergibt sich aus der Rail manual; die OPA-Strukturprüfung und der Daily-Cap bleiben aktiv. Das Skript `scripts/ops/psbt_submit.sh`reicht eine mit Sparrow (Watch-Only-Wallet) erstellte PSBT aus einem Verzeichnis über diesen Weg ein.
...
3. Operator exportiert die PSBT auf das USB-Medium:
   ```bash
   scripts/ops/refill/psbt_export.sh
   ```
...
5. Operator liest die signierte PSBT zurück ein:
   ```bash
   scripts/ops/refill/psbt_broadcast.sh
   ```
...
`scripts/ops/setup/rotate_secrets.sh`rotiert die zur Laufzeit erzeugten Geheimnisse im laufenden Betrieb und zieht die Änderungen direkt nach. Rotiert werden die NATS-Passwörter, `OPERATOR_NATS_PASS`, `NTFY_TOKEN`, das `mw_app`- und das Superuser-Passwort sowie die btc-core-`rpcauth`-Credentials. Vor jeder Änderung werden `.env`und `rpcauth.conf`mit Zeitstempel gesichert.
...
```bash
scripts/ops/psbt_submit.sh /pfad/zum/sparrow-export
```

Sucht genau eine `*.psbt`(Single-TX-Regel) im angegebenen Verzeichnis, erkennt Binär- vs. Base64-Format, berechnet den SHA-256 über die dekodierten Bytes und published sie als NATS-Subject psbt.submit.requested unter der Operator-Identität (Rail manual, Whitelist-Bypass).
