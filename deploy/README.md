...
> **Zielbild (kurz):**
>
> *   **Hot‑Flow (extern, vollautomatisch):** Externe Requests → OPA Policy → **Policy‑Signer signiert PSBT** → Bitcoin Core (node-only) broadcastet
> *   **Cold‑Flow (manual refill):** Threshold triggert Refill‑Intent → Notification (ntfy, menschlicher Fallback) → Mensch exportiert PSBT auf USB → **Air‑gapped Approval + Signing** (Signer + KeyB/KeyC) → final PSBT → Broadcast
...
*   `NOTIFIER_URL` → ntfy endpoint (benachrichtigt Menschen, required fallback bei ausgefallenen Automatisierungen)
...
*   `HOT_SIGNING_KEY` / `HOT_SIGNING_PUBKEY` from Bitwarden via ExternalSecret
...
*   `POST /api/v1/sign` → returns `signed_psbt_base64`

**Note:** `policy-signer` only signs after successful OPA allow, so only policy-approved PSBT inputs are signed.
***
...
*   `contracts/openapi/policy-signer.yaml`
    *   `POST /api/v1/sign` → sign unsigned PSBT and return signed PSBT base64
...
5.  `middleware` (or `tx-builder`) triggers **notify_human** via Notifier interface; this is the required human fallback when automated flows cannot complete or refill threshold actions occur.
...
This repo now contains a PSBT-based hot wallet flow with the following current state:

*   `middleware` now supports PSBT hot transaction requests and PSBT signing orchestration via `policy-signer`.
*   `tx-builder` is the refill intent and PSBT builder component; it still needs production-grade UTXO tracking and fee estimation when moving beyond the regtest prototype.
*   `policy-signer` now accepts `unsigned_psbt_base64`, applies OPA-based policy gating, and returns `signed_psbt_base64`.
*   Hot signing keys are expected from Bitwarden via the `btc-hot-signing-keys` ExternalSecret.

Planned next iterations:

*   Finalize DB schema (intents, tx lifecycle, audit, idempotency)
*   Harden `policy-signer` AWS KMS/HSM signing backend and key binding
*   Complete event lifecycles for NATS JetStream stream/consumer definitions
*   Validate log collection and alerting for `btc-hot` services in Loki/Grafana
